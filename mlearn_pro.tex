...
\begin{problem}
	Как должны быть связаны матрицы $A$ и $B$, чтобы равенство $\langle Ax, y\rangle = \langle A, By \rangle$ выполнялось для любых векторов $x$ и $y$?
...
=======
Известно, что $UU^T= I$, где $I$ — единичная матрица. Докажите, что 
>>>>>>> bff4da11f63c063771bb6590640438e2cc7aeadd
\begin{enumerate}
	\item $U^TU = I$;
...
\begin{problem}
  Найдите такой вектор $w$, чтобы отражение относительно плоскости, ортогональной вектору $w$, переводило вектор $(2, 5, -3)'$ в вектор $(a, 0, 0)$. Выпишите матрицу соответствующего отражения.

\begin{sol}
  Замечаем, что отражение сохраняет длины, поэтому $a^2 = 2^2 + 5^2 + (-3)^2$. А дальше берём $w = (2, 5, -3)' - (a, 0, 0)'$, так как эта разница ортогональна плоскости, относительно которой выполняется отражение. 
