...
\begin{problem}
На плоскости расположены колонии рыжих и чёрных муравьёв. Рыжих колоний три и они имеют координаты $(0, 0)$, $(1, 1)$ и $(3, 3)$. Чёрных колоний тоже три и они имеют координаты $(2, 2)$, $(4, 4)$ и $(5, 5)$.
...
\begin{problem}
На плоскости расположены три точки, $A=(0, 0)$, $B=(3, 4)$ и $C=(6, 0)$. Джеймс Бонд равновероятно забрасывается в одну из трёх точек, а затем перемещается в одну из оставшихся, чтобы запутать следы. Вероятность перемещения из точки заброски в точку $X$ пропорциональна $f(d)$, где $d$ — расстояние от точки заброски до точки $X$, а $f$ — функция плотности нормального распределения $\cN(0;\sigma^2)$.

Майор Пронин устроил засады на всех трёх дорогах: $AB$, $BC$ и $AC$.

\begin{enumerate}
  \item Найдите вероятность поимки Джеймса Бонда на каждой из дорог для $\sigma=1$, $\sigma \to 0$, $\sigma \to \infty$.
  \item Какие возможные значения принимает спутанность (perplexity) распределения выбираемой дороги при произвольных $\sigma$?
  \todo[inline]{проверить! интуитивно должны быть экстремумы на краевых $\sigma$}
  \item Какова вероятность поимки Джеймса Бонда на каждой из дорог для $\sigma=1$, $\sigma \to 0$, $\sigma \to \infty$, если по последним разведданным он был заброшен в точку $A$?
\end{enumerate}
...
\begin{enumerate}
\item Какова вероятность поимки Джеймса Бонда на каждой из дорог, если $f(a) = 1/a$? $f(a)$ — стандартная нормальная функция плотности? $f(a)$ — функция плотности распределения Коши?
\item При какой из трёх указанных $f$ дивергенция
\end{enumerate}
