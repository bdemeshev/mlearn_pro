...
\usepackage{minted} % оформление кода, требуется питон за кадром
\usepackage{caption} % для вставки \captionof внутри окружения minipage без окружения figure
...
\begin{problem}
Бандерлог утверждает, что открыл новую дифференциируемую верхнюю границу для пороговой функции потерь,
...
\begin{problem}
Бандерлог из Лога оценил логистическую регрессию $\ln \frac{\hat\P(y_i=1)}{\hat\P(y_i=0)} = 2 - x + 3z$.

В осях $(x, z)$ нарисуйте линии $\hat\P(y_i = 1)=0.5$, $\hat\P(y_i = 1)=0.99$ и $\hat\P(y_i=0.01)$

\begin{sol}
$\hat\P(y_i = 1)=0.5$ в нужных осях является прямой: $2-x+3z=0$.
\end{sol}
\end{problem}


\begin{problem}
Какие утверждения верны для логистической регрессии с константой?
\begin{enumerate}
  \item $\sum_{i=1}^n \hat \P(y_i = 1) = \text{количество } y_i { в выборке, равных единице}$
  \item $\sum_{i=1}^n \hat \P(y_i = 0) = \text{количество } y_i { в выборке, равных нулю}$
  \item $\sum_{y_i=1} \hat \P(y_i = 0) = \sum_{y_i=0} \hat \P(y_i = 1)$
  \item $\sum_{y_i=0} \hat \P(y_i = 0) = \sum_{y_i=1} \hat \P(y_i = 1)$
\end{enumerate}
...
\section{Разложения матриц и немного линейной алгебры}

\begin{problem}
	Как должны быть связаны матрицы $A$ и $B$, чтобы равенство $\langle Ax, y\rangle = \langle x, By \rangle$ выполнялось для любых векторов $x$ и $y$?
\begin{sol}
$B = A^T$
\end{sol}
\end{problem}
...
\begin{problem}
Известно, что $UU^T= I$, где $I$ — единичная матрица. Докажите или опровергните утверждения:
\begin{enumerate}
	\item $U^TU = I$;
	\item Длина векторов $x$ и $Ux$ всегда одинаковая.
	\item Угол между векторами $x$ и $y$ такой же, как между векторами $Ux$ и $Uy$;
\end{enumerate}
\begin{sol}
	$(Ux)'Ux=x'U'Ux=x'x$, $(Ux)'Uy=x'U'Uy=x'y$.
\end{sol}
\end{problem}
...
\begin{problem}
  Посмотрите на ходулочника в воде :)

\begin{minipage}{0.6\textwidth}
\begin{center}
\includegraphics[scale=0.6]{images/Black-necked_Stilt_Corte_Madera.jpg}
\end{center}
\captionof{figure}{\href{https://commons.wikimedia.org/w/index.php?curid=29091464}{Frank Schulenburg, CC BY-SA 3.0}}

  \end{minipage}


  \begin{enumerate}
    \item Найдите матрицу, которая выполняет отражение относительно плоскости, ортогональной вектору $w'=(1, 2, 3)$.
    \item Как будет выглядеть матрица отражения $U$ относительно гипер-плоскости ортогональной вектору $w$ в общем случае?
    \item Пусть $U$ матрица произвольного отражения. Верно ли, что $U^{T}=U$? Верно ли, что $U^2=I$?
  \end{enumerate}
\begin{sol}
  $U=I - \frac{2}{w'w}ww'$, $U^2=I$ и $U'=U$.
\end{sol}
\end{problem}


\begin{problem}
  Найдите такой вектор $w$, чтобы отражение относительно плоскости, ортогональной вектору $w$, переводило вектор $(4, 2, 4)'$ в вектор $(a, 0, 0)$, $a>0$.
  Выпишите матрицу соответствующего отражения.

\begin{sol}
  Замечаем, что отражение сохраняет длины, поэтому $a^2 = 4^2 + 2^2 + 4^2$.
  А дальше берём $w = (4, 2, 4)' - (a, 0, 0)'$, так как эта разница ортогональна плоскости, относительно которой выполняется отражение.
  Матрица отражения равна $I - \frac{2}{w'w}ww'$.
\end{sol}
\end{problem}
...
\[
  ...
  \end{pmatrix}
\]
Найдите такую последовательность отражений, которая превращает матрицу $X$ в бидиагональную матрицу $\Sigma$.
То есть у матрицы $\Sigma$ только на главной диагонали и на линии выше главной диагонали (супер-диагонали) могут стоять ненулевые числа.
Отражения можно применять слева и справа:
\[
X = U_2 U_1 \Sigma V_1'
\]

Отражение $U_1$ должно занулять элементы под главной диагональю в первом столбце.
Затем отражение $V_1'$ должно занулять элементы правее супер-диагонали в первой строке, не портя ранее занулённых элементов.
Затем отражение $U_2$ должно занулять элементы под главной диагональю во втором столбце, не портя ранее занулённых элементов.
\begin{sol}
  $I - \frac{2}{w'w}ww'$
\end{sol}
\end{problem}
...
\begin{problem}
  Как выглядит матрица $U$, поворачивающая двумерные вектора на угол $\alpha$ по часовой стрелке? Как связаны $U^T$ и $U^{-1}$?
\begin{sol}
  \[
    U = \begin{pmatrix}
      \cos \alpha & \sin \alpha \\
...
\begin{problem}
Пусть $y$ и $x$ — это два произвольных вектора. Найдите
\begin{enumerate}
  \item Проекцию вектора $y$ на прямую, порождённую вектором $x$;
  \item Проекцию вектора $y$ на плоскость, ортогональную вектору $x$;
  \item Отражение вектора $y$ относительно плоскости, ортогональной вектору $x$;
\end{enumerate}
\begin{sol}
\end{sol}
\end{problem}
...
\begin{problem}
Исследователь Геродот получил сингулярное разложение матрицы $X$,
\[
X = U \cdot
\begin{pmatrix}
  7 & 0 & 0 \\
...
Если возможно, найдите сингулярное разложение для
\begin{enumerate}
  \item $X'$, $10X$;
  \item $X'X$, $XX'$;
  \item $(X'X)^4$, $(XX')^{-1}$, $5I + X'X$;
  \item $X'(XX')^{-1}X$;
  \item $X(X'X)^{-1}X'$.
\end{enumerate}
\begin{sol}
...
\begin{problem} % уточнить, что K1 = 0 иначе
Пусть $x$ и $z$ — строки символов, возможно разной длины. Рассмотрим две функции. Функция $K_1(x, z)$ равна единице, если строки $x$ и $z$ совпадают. Функция $K_2(x, z)$ — число совпадающих подстрок. Функция $K_3$ — произведение количеств букв «а» в обоих словах.
\begin{enumerate}
...
\begin{sol}
\begin{enumerate}
\item $K_1(\text{«мама»},\text{«ам»}) = 0$, $K_2(\text{«мама»},\text{«ам»}) = 3$, $K_3(\text{«мама»},\text{«ам»}) = 2$
\item Да
\item Да
\item
\end{enumerate}
...
\begin{problem}
На прямой аллее растёт три дуба. Находятся в точках с координатами $x_1 = 1$, $x_2 = 2$ и $x_3 = -3$. Исследователь Винни-Пух проверил и выяснил, что на первом Дубе водятся правильные пчёлы, а на остальных — неправильные.
...
\begin{problem}
Стомерный Морской Ёж всплывает со дна стомерной пучины. У Стомерного Морского Ежа ровно сто иголок. Длина каждой иголки равна единице, а угол между любыми двумя иголками ровно прямой. Проблема в том, что 42 иголки повреждены. Чтобы повреждённые иголки регенерировались, Стомерному Морскому Ежу необходимо всплыть так, чтобы повреждённые иголки оказались над водой, а целые — под водой.

\begin{enumerate}
  \item При любом ли расположении повреждённых игл Стомерный Морской Ёж сможет их регенерировать?
  \item Верно ли, что гауссовское ядро $K(x, x')=\exp(-\gamma ||x-x'||^2)$ позволяет идеально разделить любые наблюдения кроме совпадающих при достаточно большом параметре $\gamma$?
\end{enumerate}

\begin{sol}
  Да, при любом. Подходящую плоскость можно задать так: взять $x_i$ с номерами повреждённых иголок с плюсом, а $x_i$ с номерами целых иголок — с минусом. И сравнить эту сумму с нулём.

  Да, при большом $\gamma$ гауссовское ядро превращает любой вектор в вектор единичной длины и спрямляет углы. То есть набор данных из $n$ наблюдений с ростом $\gamma$ можно считать $n$-мерным Морским Ежом :)
\end{sol}
\end{problem}
...
\[
\begin{cases}
\frac{1}{2} \left\Vert w \right\Vert^2 + C \sum_{i=1}^l \xi_i \to \min_{w, b, \xi} \\
y_i (\langle w, x_i \rangle + b) \geq 1 - \xi_i, \quad i=1 \ldots l, \\
...
0 \leq \lambda_i \leq C, \quad i=1 \ldots l, \\
...
\begin{problem}
У бесстрашного исследователя Василия есть набор точек $(x_1, y_1)$, \ldots, $(x_n, y_n)$.
Василий находит прямую, сумма расстояний от которой до каждой точки минимальна.

Верно ли, что прямая проходит через точку $(\bar x, \bar y)$?
...
\begin{sol}
  Те же компоненты, только в обратном порядке.
\end{sol}
\end{problem}
...
\begin{sol}
$I_X = 1 - 0.2^2 - 0.8^2 = 0.32$, $H(X) = -(0.2 \ln 0.2 + 0.8 \ln 0.8) \approx 0.5$, preplexity $\approx e^{0.5}$
...
\begin{problem}
Как изменится энтропия дискретной величины $X$, если величину домножить на 10?
А если у величины $X$ есть функция плотности?
\begin{sol}
...
\begin{problem}
Величины $X$ и $Y$ независимы, и являются компонентами вектора $V$, $V=(X, Y)$.
Как связаны энтропия $V$ и энтропии $X$ и $Y$?
\begin{sol}
\[
H(V) = H(X) + H(Y)
\]
...
\begin{problem}
Дискретные величины $X$ и $Y$ независимы, и являются компонентами вектора $V$, $V=(X, Y)$.
Как связаны индекс Джини $V$ и индексы Джини $X$ и $Y$?
\begin{sol}
...
\begin{problem}
Машин-лёрнер Василий собрал три дискретных наблюдения $y_i$ и придумал
структуру Очень Глубокой нейронной сети, выдающей прогнозы вероятности
$\P(y_i = 1)$ в зависимости от вектора Глубоких параметров $\theta$ и значений предикторов $x_i$.
...
\begin{enumerate}
  \item Машин-лёрнер Василий использует метод максимального правдоподобия.
  Как выглядит целевая функция машин-лёрнера Василия?
  \item Кот машин-лёрнера Василия использует кросс-энтропию из аппроксимирующего
  распределения, задаваемого нейронной сетью, в истинные значения $y_i$.
  Как выглядит выглядит целевая функция кота Василия?
\end{enumerate}

\begin{sol}
Функции отличаются знаком. Кросс-энтропия равна минус логарифму правдоподобия.
\end{sol}
\end{problem}
...
Пятачок использует квадратичную аппроксимацию для функции правдоподобия в точке $w=0.5$:
\[
Obj(w) = \sum_{i=1}^n \left(loss(y_i, 0.5) +
loss'_{w}(y_i, 0.5) (w_i - 0.5) +
\frac{1}{2} loss''_{ww}(y_i, 0.5) (w_i - 0.5)^2 \right) + \frac{1}{2} \lambda |w|^2.
\]


Помогите Очень Маленькому Существу подобрать оптимальные величины $(w_i)$ при $\lambda = 1$.

\todo[inline]{Отметить, что «да» — влево}
...
\begin{problem}
Кот Леопольд анкетировал 10 мышей по трём вопросам: $x$ — «Одобряете ли Вы непримиримую к котам позицию Белого и Серого?», $y$ — «Известно ли Вам куда пропала моя любимая кошка Мурка?» и $z$ — «Известны ли Вам настоящие имена Белого и Серого?» Результаты опроса в таблице:


\todo[inline]{Сюда табличку!}
...
\begin{sol}
  Сначала делим по условию $x>0.5$, затем по условию $z>1.5$.
\end{sol}
\end{problem}


\begin{problem}
В ходе применения алгоритма \verb|xgboost| машин-лёрнер Василий построил дерево.
Для каждого узла указано изменение индекса Джини. Машин-лёрнер Василий хочет подрезать
дерево так, чтобы минимизировать финальный индекс Джини с учётом штрафа $0.05$
за каждое деление узла на два.

Как следует Василию подрезать дерево?
...
\tikzstyle{level 3}=[sibling distance=0.4cm]

\node[mystart, label=right: {$-0.2$}] {} % создаем узел типа mydot, с пометкой "N" сверху и пустым текстом внутри узла
    child { node[mydot, label=left: {$-0.02$}] (a) {}
                	child {	node[mydot, label=left: {$-0.06$}] {}
                    child { node[mydot, label=below: {}] {}
                    edge from parent node[left] {} }
                    child { node[mydot, label=below: {}] {}
                    edge from parent node[right] {} }
                	edge from parent node[left] {}}
                	child {	node[mydot, label=left: {$-0.03$}] {}
                    child { node[mydot, label=below: {}] {}
                    edge from parent node[left] {} }
                    child { node[mydot, label=below: {}] {}
                    edge from parent node[right] {} }
    ...
    child { node[mydot, label=right: {$-0.01$}] {}
                	child {	node[mydot, label=right: {$-0.1$}] {}
                    child { node[mydot, label=below: {}] {}
                    edge from parent node[left] {} }
                    child { node[mydot, label=below: {}] {}
                    edge from parent node[right] {} }
                	edge from parent node[left] {}}
                	child {	node[mydot, label=right: {$-0.04$}] {}
                    child { node[mydot, label=below: {}] {}
                    edge from parent node[left] {} }
                    child { node[mydot, label=below: {}] {}
                    edge from parent node[right] {} }
...
\end{tikzpicture}
\begin{sol}
  Подрезаем дерево с конца по методу обратной индукции.
...
\begin{problem}
Постройте пример, в котором индекс Джини больше $0.99$.
\begin{sol}
В мешке лежит 1000 наблюдений, все разных типов!
\end{sol}
\end{problem}

\section{Бэггинг и бустинг}
...
$\P(S = s)$ & $\frac{1}{27}$ & $\frac{3}{27}$ & $\frac{3}{27}$ & $\frac{4}{27}$ & $\frac{6}{27}$ & $\frac{3}{27}$ & $\frac{3}{27}$ & $\frac{3}{27}$ & $\frac{1}{27}$ \\ \bottomrule
\end{tabular}
\item $\E(X) = 700$, $\Var(X) = 14000/3$
\item
\begin{itemize}
\item $S = \max\{x_1, x_2, x_3\}$

\begin{tabular}{@{}lccc@{}}
\toprule
$s$         & $300$          & $600$          & $1200$           \\
$\P(S = s)$ & $\frac{1}{27}$ & $\frac{7}{27}$ & $\frac{19}{27}$ \\ \bottomrule
\end{tabular}

\item $S = \min\{x_1, x_2, x_3\}$

\begin{tabular}{@{}lccc@{}}
\toprule
$s$         & $300$          & $600$          & $1200$           \\
$\P(S = s)$ & $\frac{19}{27}$ & $\frac{7}{27}$ & $\frac{1}{27}$ \\ \bottomrule
\end{tabular}
\end{itemize}
\end{enumerate}
\end{sol}
...
\begin{enumerate}
\item Какие прогнозы внутри обучающей выборки получит Василий с помощью своего леса?
\item Сколько деревьев имеет смысл посадить Василию, чтобы получить хорошие вневыборочные прогнозы по четырём наблюдениям?
\end{enumerate}
...
\begin{enumerate}
\item Какие прогнозы внутри обучающей выборки получит Василий при $\eta=1$ и $\lambda=1$?
\item Какие прогнозы внутри обучающей выборки получит Василий при $\eta=0.5$ и $\lambda=1$?
\item Сколько деревьев имеет смысл посадить Василию, чтобы получить хорошие вневыборочные прогнозы по четырём наблюдениям?
\end{enumerate}
...
\section{Разложение на шум-смещение-разброс}


\begin{problem}
Истинная зависимость имеет вид $y_i = x_i^2 + u_i$, где $y_i$ — прогнозируемая переменная, $x_i$ — предиктор и $u_i$ — ненаблюдаемая случайная составляющая. Величины $x_i$ независимы и равновероятно принимают значения $1$ и $2$. Величины $u_i$ независимы и равновероятно принимают значения $-1$ и $1$. Начинающий машин-лёрнер Василий может позволить себе обучающую выборку только из двух наблюдений.
...
\begin{sol}
\begin{enumerate}
  \item Обозначим $x_*$ — значение аргумента, для которого мы строим прогноз.
  Формула, по которой Василий строит прогноз, $\hat y(x_*) = 0$. Сначала зафиксируем
  значение $x_*$.
  Получаем разложение:
  \[
     \E((\hat y_* - y_*)^2|x^*) = \Var(\hat y_* | x_*) +
     (\E( \hat y_* - y_* |x^*))^2 +
     \E(u_*^2|x_*)
  \]
  В нашем случае оно превращается в
 \[
    \E((\hat y_* - y_*)^2|x_*)   =  0 + x_*^4 + 1
 \]
 При желании, можно получить безусловный вариант разложения,
 взяв от всего ещё поверх математическое ожидание:
 \[
    \E((\hat y_* - y_*)^2) = 0 + 17/2  + 1
 \]
\item
...
Разложение в общем виде:
  \[
     ...
     \E(u_*^2|x_*)
  \]
Отдельно находим каждую часть:
\[
\E(u_*^2|x_*)=\E(u_*^2)= 1
\]
\[
\Var(\hat y_*|x_*)=\Var(y_2|x_*)=\Var(y_2)=\Var(x_2^2) + \Var(u_2)=\frac{9}{4}+1
\]
\[
\E(\hat y_* - y_*|x_*)=\E(y_2|x_*)-\E(y_*|x_*)=\E(y_2) - (x_*^2 + 0) = 2.5 - x_*^2
\]
В результате получаем:
 \[
    \E((\hat y_* - y_*)^2|x_*)   =  3.25 + (2.5 - x_*^2)^2 + 1
 \]
 При желании, можно получить безусловный вариант разложения,
 взяв от всего ещё поверх математическое ожидание:
 \[
    \E((\hat y_* - y_*)^2) = 3.25 + 2.25  + 1
 \]
\end{enumerate}
...
\begin{problem}
Истинная зависимость имеет вид $y_i = 3x_i^2 + u_i$, где $y_i$ — прогнозируемая переменная, $x_i$ — предиктор и $u_i$ — ненаблюдаемая случайная составляющая. Величины $x_i$ независимы и равновероятно принимаю значения $0$, $1$, $2$. Величины $u_i$ независимы и равновероятно принимают значения $-1$ и $1$.
...
\begin{problem}
Количество мёда в горшках, которое Винни-Пух хочет съесть в гостях, можно представить в виде $y_i = 5 + u_i$, где $u_i$ имеет равномерное распределение $u_i \sim \mathcal{U}[-2;2]$, а $i$ — номер визита в гости.
...
\begin{problem}
Вася измерил вес трёх покемонов, $y_1=6$, $y_2=6$, $y_3=10$. Вася хочет спрогнозировать вес следующего покемона. Модель для веса покемонов у Васи очень простая, $y_i = \mu + u_i$, поэтому прогнозирует Вася по формуле $\hat y_i = \hat\mu$.
...
\begin{sol}
  \[
    \E((y_f - \hat y_f)^2)= 1 + \gamma^2 x_f^2 \frac{1}{\sum x_i^2} + \beta^2 x_f^2 (1-\gamma)^2
  \]
  Оптимальное $\gamma$ равно
  \[
    \gamma = \frac{\beta^2\sum x_i^2}{\beta^2\sum x_i^2 + 1}
  \]
  На практике эта формула в явном виде неприменима, так как содержит неизвестный параметр $\beta$. Способ 1: двухшаговая оценка. Оцениваем $\hb$ с помощью МНК, затем находим $\hat\gamma$ используя $\hb$ вместо $\beta$. Способ 2: кросс-валидация. Минимизируем сумму квадратов вневыборочных прогнозов по $\gamma$.
\end{sol}
\end{problem}
...
\begin{problem}
  Исследователь Януарий хочет поделить $1$ на $7$ с точностью до $10^{-10}$.
  Проблема в том, что делить в столбик Януарий не умеет, а умеет только умножать, складывать и вычитать.
  Разработайте для Януария итерационный алгоритм вида $x_{n} = f(x_{n-1})$,
  который при старте с любого $x_0 \in (0;2/7)$ бодро сходится к $1/7$.
\begin{sol}
  Например, можно рассуждать так. Мы хотим, чтобы расстояние от $x_n$ до $1/7$ с ростом сокращалось, поэтому нам пойдет например, алгоритм
  \[
     x_n - 1/7 = (x_{n-1} - 1/7) / 42
  \]
  Проблема с этим алгоритмом в том, что он требует делений.
  Приходит в голову идея заменить деление на $42$ на возведение в квадрат, тогда при малом расстоянии оно будет и дальше сокращаться:
  \[
...
\begin{problem}
Исследователь Антоний ищет корень уравнения $x^3 + 7x - 3=0$.
Заметив, что в единице и нуле значения многочлена имеют разный знак, Антоний использует метод деления отрезка пополам.
Помогите Анонию найти корень уравнения с точностью до $1/8$.
...
\begin{problem}
Исследователь Александр оценивает множественную регрессию $y_i = \beta_1 + \beta_2 x_i + \beta_3 z_i + u_i$ с помощью МНК.
Все регрессоры, включая единичный столбец, он поместил в матрицу $X$.
Оказалось, что
\[
  X'X = \begin{pmatrix}
    100 & 5 & -10 \\
...
\begin{pmatrix}
  ...
  \end{pmatrix};
\]
Александр использует градиентный спуск для поиска $\hat\beta$.
Стартует с точки $\hb' = (0,0,0)$, а длина шага постоянна и равна $\gamma = 0.1$.
...
\begin{problem}
  Исследователь Аристарх ищет минимум квадратичной функции $y(x)=ax^2+bx+c$, где $a>0$, методом градиентного спуска.
  Аристарх стартует из точки $x_0$ и настолько ленив, что не хочет делать больше одного шага.

  \begin{enumerate}
    \item При каком значении длины шага $\gamma$ Аристарх за один шаг окажется точно в точке минимума?
    \item Аристарх освоил курс линейной алгебры, но остался таким же ленивым :) Теперь Аристарх хочет за одну итерацию найти минимум векторной функции
      \[
         Q(x) = x'Ax + b'x + c
       \]
       Подскажите, какую матрицу длины шага $\Gamma$ хорошо бы взять Аристарху, если он делает шаг вида $x_n = x_{n-1} - \Gamma \nabla Q(x_{n-1})$.
     \item Почему в методе Ньютона итерации делают по принципу $x_n = x_{n-1} - (H(x_{n-1}))^{-1}\nabla Q(x_{n-1})$, где $H$ — матрица Гессе целевой функции?
   \end{enumerate}

\begin{sol}
  При $\gamma = 1/2a$, $\Gamma = A^{-1}/2$.
...
\begin{problem}
  Заблудившись в лесу Иван Сусанин в отчаянии бросает карту леса на землю.
  Докажите, что на земле найдётся такая точка, которая идеально точно находится под своим изображением на карте.
  \begin{sol}
...
\begin{problem}
  Вывод BB-метода \url{http://www.math.ucla.edu/~wotaoyin/math273a/slides/Lec4a_Baizilai_Borwein_method_273a_2015_f.pdf}.
...
\begin{problem}
SGD для обычной регрессии: сделать два шага.
\begin{sol}
...
\begin{problem}
SGD для логистической регрессии: сделать два шага.
\begin{sol}
...
\begin{problem}
Метод золотого сечения? Плохие числа?
\begin{sol}
...
\begin{problem}
Нарисуйте кляксу. Нарисуйте внутри кляксы треугольник. С помощью алгоритма Нелдера-Мида найдите точку, наиболее удалённую от границы кляксы.
\begin{sol}
  \url{http://www.jasoncantarella.com/downloads/NelderMeadProof.pdf} и \url{http://www.scholarpedia.org/article/Nelder-Mead_algorithm}
\end{sol}
\end{problem}
...
\begin{problem}
Найдите производную следующей функции по матрице $W_1$ при фиксированном $x \in R^n$.

\[
L \left( W_1 \cdot f(W_2 x + b_2) + b_1 \right)
\]
где $f(x) = \max \{0, x\}$.
Выразите ответ через производную $L$ по её аргументу и значению $ f(W_2 x + b_2) $ в точке $x$. Как будет выглядеть градиентный шаг для $W_2$?
\begin{sol}
...
На выходе Soft-Max слоя мы получили матрицу размера 100 на 20 и попали в Dropout с вероятностью отключения нейрона равной 0.2. Какова вероятность, что выключится весь слой? Как можно считать градиент по Dropout слою?
\begin{sol}

\end{sol}
\end{problem}

\section{Ближайшие соседи}
...
\begin{sol}
При $k=1$ получаем 4 ошибки, при $k=3$ получаем 2 ошибки, при $k=5$ получаем ... Оптимальное $k=3$.
\end{sol}
\end{problem}
...
\begin{sol}
При трёх соседях вся плоскость под влиянием рыжих. При одном соседе область влияния чёрных — прямоугольник бесконечный влево вниз. При кросс-валидации с $k=1$ получаем $4$ ошибки, при кросс-валидации с $k=3$ получаем одну ошибку. Оптимальное $k=3$.
\end{sol}
\end{problem}
...
\[
D_{KL}(p||q) = \E(\ln p(X)) - \E(\ln q(X))
\]

Ожидание считается по истинному распределению $p()$.
...
\begin{sol}
\begin{enumerate}
\item $D_{KL} (p||q) = \left(\frac{2}{3}\right)^2 \ln \left(\frac{2}{3}\right)^2 \cdot 2 + \left(\frac{1}{3}\right)^2 \ln \left(\frac{1}{3}\right)^2 - \ln\left(\frac{1}{3}\right) = \frac{16}{9} \ln 2 - \ln 3 \approx 0.134$
\item $D_{KL} (p||q) = 3 \cdot \frac{1}{3} \ln \frac{1}{3} - \left(\frac{1}{3} \ln \left(\frac{2}{3}\right)^2 + \frac{1}{3} \ln \left(\frac{4}{9}\right) + \frac{1}{3} \ln \left(\frac{1}{3}\right)^2 \right)   = \ln 3 - \frac{4}{3} \ln 2 \approx 0.17$
\item Для удобства запишем выражение как логарифм частного:
\[
D_{KL} (p||q) = \int_{-\infty}^{+\infty} \frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{x^2}{2\sigma^2}} \ln \frac{\frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{x^2}{2\sigma^2}}}{\frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}}} dx
= \frac{1}{\sqrt{2\pi\sigma^2}} \int_{-\infty}^{+\infty}  e^{-\frac{x^2}{2\sigma^2}} \left(-\frac{x^2}{2\sigma^2} + \frac{x^2}{2} -\ln \sigma \right) dx
\]
...
\[
D_{KL} (p||q) = \int_{-\infty}^{+\infty} \frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}} \ln \frac{\frac{1}{\sqrt{2\pi}} e^{-\frac{x^2}{2}}}{\frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{x^2}{2\sigma^2}}} dx
= \frac{1}{\sqrt{2\pi}} \int_{-\infty}^{+\infty}  e^{-\frac{x^2}{2}} \left(\frac{x^2}{2\sigma^2} - \frac{x^2}{2} +\ln \sigma \right) dx
\]
\[
D_{KL} (p||q) = \ln \sigma - \frac{1}{2} + \frac{1}{2\sigma^2}
\]
\todo[inline]{здесь не существует?}
\end{enumerate}
\end{sol}
\end{problem}
...
\begin{problem}
  Докажите, что дивергенция Кульбака-Лейблера из аппроксимирующего $q$ в истинное распределение $p$ неотрицательна.
\begin{sol}
Заметим, что
\[
...
\begin{problem}
На плоскости расположены три точки, $A=(0, 0)$, $B=(3, 4)$ и $C=(6, 0)$. Джеймс Бонд равновероятно забрасывается в одну из трёх точек. Затем Джеймс Бонд перемещается в одну из оставшихся точек, чтобы спутать следы. Вероятность перемещения из точки заброски $Z$ в точку $X$ пропорциональна $f(d)$, где $d$ — расстояние от точки заброски $Z$ до точки $X$, а $f$ — функция плотности нормального распределения $\cN(0;\sigma^2_Z)$. Параметр $\sigma^2_Z$ — это сила, оставшаяся у Джеймса-Бонда после заброски в точку $Z$.

Майор Пронин устроил засады на всех трёх дорогах: $AB$, $BC$ и $AC$.

\begin{enumerate}
  \item Какова вероятность поимки Джеймса Бонда на каждой из дорог для $\sigma_A=1$, $\sigma_A \to 0$, $\sigma_A \to \infty$, если по последним разведданным он был заброшен в точку $A$?
  \item Какие возможные значения принимает спутанность (perplexity) распределения выбираемой дороги при произвольных $\sigma_A$, если известно, что Джеймс Бонд был заброшен в $A$? Хватит ли у Джеймса Бонда сил, чтобы обеспечить спутанность равную 3 в точке $A$?
  \item Какие возможные значения принимает спутанность (perplexity) распределения выбираемой дороги при произвольных $\sigma_B$, если известно, что Джеймс Бонд был заброшен в $B$?
  \item Найдите вероятность поимки Джеймса Бонда на каждой из дорог для случая равных $\sigma_Z$: $\sigma_Z=1$, $\sigma_Z \to 0$, $\sigma_Z \to \infty$ и неизвестной точки заброски.
\end{enumerate}

\begin{sol}
\begin{enumerate}
\item $p_{AB} = \frac{f(5)}{f(5) +  f(6)} = \frac{1}{f(6)/f(5) + 1}$

$p_{AC} = \frac{f(6)}{f(5) +  f(6)} = \frac{1}{f(5)/f(6) + 1}$

$p_{BC} = 0$

При $\sigma_A = 1$: $p_{AB} = \frac{1}{1+e^{-5.5}} \approx 0.996$, $p_{AC} = \frac{1}{1+e^{5.5}} \approx 0.004$

При $\sigma_A \to 0$: $p_{AB} \to 1$, $p_{AC} \to 0$

При $\sigma_A \to \infty$: $p_{AB} \to 0.5$, $p_{AC} \to 0.5$
\item При $\sigma_A \to 0$ perplexity $\to 1$

При $\sigma_A \to \infty$ perplexity $\to 2$

\item Если Джеймс Бонд заброшен в точку $B$, то он выбирает дороги $AB$ и $BC$ равновероятно, не зависимо от $\sigma_B$. Спутанность равна 2.
\item При неизвестной точке заброса вероятности равны:
\[
...
При $\sigma_Z \to 0$: $p_{AB} \to 0.5$, $p_{BC} \to 0.5$, $p_{AC} \to 0$

При $\sigma_Z \to \infty$: $p_{AB} \to 1/3$, $p_{BC} \to 1/3$, $p_{AC} \to 1/3$
\end{enumerate}
\end{sol}
\end{problem}
...
\begin{problem}
На плоскости расположены три точки, $A=(0, 0)$, $B=(3, 4)$ и $C=(3, 0)$. Джеймс Бонд равновероятно забрасывается в одну из трёх точек, а затем перемещается в одну из оставшихся, чтобы запутать следы. Вероятность перемещения из точки заброски в точку $X$ пропорциональна $f(d)$, где $d$ — расстояние от точки заброски до точки $X$.
...
\begin{enumerate}
\item Какова фактическая вероятность поимки Джеймса Бонда на каждой из дорог, если $f(a) = 1/a$? $f(a)$ — стандартная нормальная функция плотности? $f(a)$ — функция плотности распределения Коши?
\item Какова вероятность поимки Джеймса Бонда на каждой из дорог по мнению майора Пронина?
\item Найдите дивергенцию Кульбака Лейблера из аппроксимирующих вероятностей по мнению Пронина в истинные вероятности появления Джеймса Бонда на каждой из дорог для случая $f(a)=1/a$.
\end{enumerate}
...
\]
\item Из условия: «Майор Пронин считает, что пункты $A$, $B$ и $C$ равноудалены друг от друга», следовательно, $p_{AB} = p_{AC} = p_{BC} = 1/3$.
\item $D_{KL} (p||q) = \frac{67}{168} \ln \frac{67}{168} + \frac{62}{189} \ln \frac{62}{189} +  \frac{59}{216} \ln  \frac{59}{216} + \ln 3 \approx 0.012$
\end{enumerate}
\end{sol}
\end{problem}
...
\begin{problem}
Рассмотрим факторизационную машину с прогнозом
\[
...
\begin{enumerate}
\item Сколько параметров имеет эта модель?
\item Какое $r$ является теоретически максимально возможным для заданного $d$?
\item Сколько ограничений нужно добавить к задаче обыкновенного МНК с регрессорами $w_j$ и $w_j w_k$ с $k>j$, чтобы она была эквивалентна факторизационной машине? Будут ли эти ограничения на параметры МНК линейными?
\item Найдите градиент $a(x)$ по вектору $v_j$.
\end{enumerate}
...
\begin{problem}
Пусть $W = V\cdot V'$ и матрица $V$ имеет размер $d \times 1$.

\begin{enumerate}
  \item Найдите матрицу $W$, если $V_{j1}=j$ и $d=3$;
  \item Является ли матрица $W$ положительно определённой? Положительно полуопределённой?
  \item Найдите собственные числа и собственные векторы матрицы $W$;
\end{enumerate}
...
3 & 6 & 9 \\
\end{pmatrix}$
\item Является положительно полуопределённой
\item $\lambda_1 = 14$, $\lambda_2 = \lambda_3 = 0$, $v_1 = (1, 2, 3)$, $v_2 = (-3, 0, 1)$, $v_3 = (-2, 1, 0)$
\end{enumerate}
\end{sol}
\end{problem}
...
\begin{problem}
Найдите разложение матрицы $W$ вида $W=V\cdot V'$ с матрицей $V$ минимального размера:
\begin{enumerate}
  \item $W = \begin{pmatrix}
   2 & 2 \\
   2 & 2 \\
...
\begin{problem}
Рассмотрим классический алгоритм PageRank с единичной вероятностью продолжать клики. Каждый пользователь стартует со случайной равновероятно выбираемой страницы. Затем пользователь равновероятно переходит по одной из ссылок на следущую страницу. И так далее.

\begin{tikzpicture}
    \node (X) at (0,0) {x};
    \node (Y) [below=2cm of X]  {y};% 2cm below, 1cm to the left (optional)
    \node (Z) [right=2cm of X] {z};
    \node (U) [below=2cm of Z]  {u};
    \draw [semithick,->] (X) -- (Y);
    \draw [semithick,->] (X) -- (Z);
    \draw [semithick,->] (Y) -- (Z);
    \draw [semithick,<->] (Y) -- (U);
